...
class WindowAttention(layers.Layer):
    ...
    def get_config(self):
        ...
        config.update({
            'dim_wa': self.dim,
            'window_size_wa': self.window_size,
            'num_heads_wa': self.num_heads,
        })
        return config
...
class SwinTransformer(layers.Layer):
    ...
    def get_config(self):
        config = super(SwinTransformer, self).get_config().copy()
        config.update({
            'dim_st': self.dim,
            'num_patch_st': self.num_patch,
            'window_size_st': self.window_size,
            'num_heads_st': self.num_heads,
            'shift_size_st': self.shift_size,
            'num_mlp_st': self.num_mlp,
        })
        return config
...
class PatchExtract(layers.Layer):
    def __init__(self, patch_size, name=None, **kwargs):
        super(PatchExtract, self).__init__(name=name)
...
class PatchEmbedding(layers.Layer):
    def __init__(self, num_patch, embed_dim, name=None, **kwargs):
...
class PatchMerging(tf.keras.layers.Layer):
    def __init__(self, num_patch, embed_dim, name=None):
    ...
    def get_config(self):
        ...
        config.update({
            'num_patch_pm': self.num_patch,
            'embed_dim_pm': self.embed_dim,
        })
        return config    
...
def get_swinT():
    ...
    crop = layers.experimental.preprocessing.RandomCrop(image_dimension, image_dimension, name='Crop')(input)
    flip = layers.experimental.preprocessing.RandomFlip("horizontal", name='Flip')(crop)
    extract = PatchExtract(patch_size, name='Extract')(flip)
    embedding = PatchEmbedding(num_patch_x * num_patch_y, embed_dim, name='Embedding')(extract)
    transformer1 = SwinTransformer(
        dim=embed_dim,
    ...
    merging = PatchMerging((num_patch_x, num_patch_y), embed_dim=embed_dim, name='Merging')(transformer2)
    gapolling = layers.GlobalAveragePooling1D()(merging)
    output = layers.Dense(num_classes, activation=None,)(gapolling)
...
def run_experiment(ldmn=0, model_number=0, lmen=1):
    ...
    outdir = f"/media/cml/Data1/jsp/LMEpredict/xrcs_SwinT/swint_test/{ldmn}/{lmen}"
    ...
    model.save_weights(f"{filepath}/model{model_number}.h5")
    ...
    with open(f"{filepath}/model{model_number}_summary.md", 'w') as f:
        with redirect_stdout(f):
            model.summary()
    return history, fcst, model

for ld in np.arange(1):
    for lme in np.arange(1,67):
        outdr = f'/media/cml/Data1/jsp/LMEpredict/xrcs_SwinT/swint_test/{ld}/{lme}'
        predict = []
        for md in np.arange(1):
