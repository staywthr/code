...
def run_experiment(ldmn=0, model_number=0, lmen=1):
    ...
    chl_valid_x = np.load(f"{indir}/chl_historical_val_x.npy")
    # val_x2 = np.load(f"{indir}/chl_valid_x.npy")
    # chl_valid_x = np.concatenate([val_x1,val_x2], axis=0)
...
# 아웃풋 저장 디렉토리 
    outdir = f"/media/cml/Data1/jsp/LMEpredict/xrchl_his+gfdl_cm4/cnn/{ldmn}/{lmen}"
...
for ld in np.arange(1):
    for lme in np.arange(1,67):
        outdr = f'/media/cml/Data1/jsp/LMEpredict/xrchl_his+gfdl_cm4/cnn/{ld}/{lme}'
        predict = []
