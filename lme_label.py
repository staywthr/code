...
def run_experiment(ldmn=0, model_number=0, lmen=1):
    ...
    train_x = np.load(f"{indir}/tr_x.npy") #pi+his
    val_x1 = np.load(f"{indir}/historical_val_x.npy") #his validation
    val_x2 = np.load(f"{indir}/valid_x.npy")
    valid_x = np.concatenate([val_x1,val_x2], axis=0)
    test_x = np.load(f"{indir}/test_x.npy")

    train_y = np.load(f"{indir}/tr_y.npy") #pi+his
    val_y1 = np.load(f"{indir}/historical_val_y.npy") #his validation
    val_y2 = np.load(f"{indir}/valid_y.npy")
    valid_y = np.concatenate([val_y1, val_y2], axis=0)

# 아웃풋 저장 디렉토리 
    outdir = f"/media/cml/Data1/jsp/LMEpredict/gfdl_cm4_pi+his+re/cnn/{ldmn}/{lmen}"
...
for ld in np.arange(1):
    for lme in np.arange(1,67):
        outdr = f'/media/cml/Data1/jsp/LMEpredict/gfdl_cm4_pi+his+re/cnn/{ld}/{lme}'
        predict = []
