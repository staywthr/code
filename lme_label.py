...
def run_experiment(ldmn=0, model_number=0, lmen=1):
    ...
    valid_x = np.load(f"{indir}/historical_val_x.npy")
...
# 아웃풋 저장 디렉토리 
    outdir = f"/media/cml/Data1/jsp/LMEpredict/xrcs_his+gfdl_cm4/cnn/{ldmn}/{lmen}"
...
for ld in np.arange(1):
    for lme in np.arange(1,67):
        outdr = f'/media/cml/Data1/jsp/LMEpredict/xrcs_his+gfdl_cm4/cnn/{ld}/{lme}'
        predict = []
