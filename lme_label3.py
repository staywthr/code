...
def get_cnn_model():
    ...
    conv1 = keras.layers.Conv2D(35, [7,7], activation='gelu', padding='same', strides=1,kernel_initializer='glorot_normal')(inputs)
    pool1 = keras.layers.MaxPooling2D((2,2), strides=2, padding='same')(conv1)
    conv2 = keras.layers.Conv2D(35, [7,7], activation='gelu', padding='same', strides=1)(pool1)
    pool2 = keras.layers.MaxPooling2D((2,2), strides=2, padding='same')(conv2)
    conv3 = keras.layers.Conv2D(35, [7,7], activation='gelu', padding='same', strides=1)(pool2)
...
# 아웃풋 저장 디렉토리 
    outdir = f"/media/cmlws/Data2/jsp/LMEpredict/cs_his+reval_k7/cnn/{ldmn}/{lmen}"
...
for ld in np.arange(1):
    for lme in np.arange(1,67):
        outdr = f'/media/cmlws/Data2/jsp/LMEpredict/cs_his+reval_k7/cnn/{ld}/{lme}'
        predict = []
