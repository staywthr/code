...
def label_generator(path, mask):
    ...
    k = list(data.keys())
    v = chk_var(k)
    maskds = xr.open_dataset("/media/cml/Data1/jsp/DATA/LME66.mask.nc").load()
...
for lmen in np.arange(1,67): 
    outdir = f"/media/cml/Data1/jsp/cmip6LMEdata/{ldmn}/{lmen}/DJF/"
    ...
    for path in (chl_valid_paths):
        ...
        frames = prepare_single_frame(path[0],ldmn)
        x_frame = frames.fillna(0)
        np.save(f'{outdir}/chl_valid_x.npy', x_frame)
        print(f'chl_valid_x.npy processed')
        ...
        label = np.array(label)
        y_label = label[:,np.newaxis]
        np.save(f'{outdir}/valid_y.npy', y_label)
        print(f'valid_y.npy processed')


    chl_test_paths = list(zip(chl_test_files, target_test_files))
    for path in (chl_test_paths):
        # Gather all its frames and add a batch dimension.
        x_basename = os.path.basename(path[0])
        x_file_name = os.path.splitext(x_basename)[0]
        ...
        frames = prepare_single_frame(path[0],ldmn)
        x_frame = frames.fillna(0)
        np.save(f'{outdir}/chl_test_x.npy', x_frame)
        print(f'chl_test_x.npy processed')
        ...
        label = np.array(label)
        y_label = label[:,np.newaxis]
        np.save(f'{outdir}/test_y.npy', y_label)
        print(f'test_y.npy processed')
    ...
    for path in (sst_valid_paths):
        ...
        frames = prepare_single_frame(path[0],ldmn)
        x_frame = frames.fillna(0)
        np.save(f'{outdir}/sst_valid_x.npy', x_frame)
        print(f'sst_valid_x.npy processed')

    sst_test_paths = list(zip(sst_test_files, target_test_files))
    for path in (sst_test_paths):
        # Gather all its frames and add a batch dimension.
        x_basename = os.path.basename(path[0])
        x_file_name = os.path.splitext(x_basename)[0]
        ...
        frames = prepare_single_frame(path[0],ldmn)
        x_frame = frames.fillna(0)
        np.save(f'{outdir}/sst_test_x.npy', x_frame)    
        print(f'sst_test_x.npy processed')
